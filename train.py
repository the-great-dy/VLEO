        ...
        if global_step % int(TRAIN_CONFIG.get("log_freq", 500)) == 0:
            ...
            log_data = {
                ...
                **_extract_training_execution_diagnostics(info),
                ...
                # ── 顶刊 Issue#1: clean CMDP 约束 vs QoS 分离日志 ──
                "constraint_total_clean": float(safety_cost.constraint_total_clean),
                "constraint_state_safety_cost": float(safety_cost.constraint_state_safety_cost),
                "constraint_drift_cost": float(safety_cost.constraint_drift_cost),
                "clean_constraint_cost_used": float(safety_cost.clean_constraint_cost_used),
                "qos_total": float(safety_cost.qos_total),
                "qos_task_loss": float(constraint_task_loss_cost),
                "qos_over_processing": float(constraint_over_processing_cost),
                "qos_low_value_waste": float(constraint_low_value_waste_cost),
                "qos_unproductive_cpu": float(constraint_unproductive_cpu_cost),
