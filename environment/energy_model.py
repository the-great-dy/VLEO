...
from safety.power_manager import apply_propulsion_deadband_watts
...
class PowerSubsystem:
    ...
    def compute_total_load(self, action: np.ndarray) -> dict:
        """
        根据动作向量计算各子系统功耗
        Args:
        ...
        P_total_unrestricted = P_prop + P_cpu + P_tx + self.P_baseline
        P_max_total = ENERGY_CONFIG.get("power_total_max_w", 120.0)
        
        # 若超限，按比例缩减（保持可调负载的相对关系）
        adjustable_power = P_prop + P_cpu + P_tx
        if P_total_unrestricted > P_max_total:
            # 总功率超限时按比例缩放三类可调负载，保持策略给出的相对优先级不变。
            scale_factor = (P_max_total - self.P_baseline) / max(adjustable_power, 1e-6)
            P_prop *= scale_factor
            P_cpu *= scale_factor
            P_tx *= scale_factor
